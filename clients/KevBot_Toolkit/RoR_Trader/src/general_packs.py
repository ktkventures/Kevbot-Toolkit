...
def _parse_pack_list(raw_packs: list) -> List[GeneralPack]:
    """Convert list of dicts to GeneralPack objects."""
    return [GeneralPack(
        id=p["id"],
...
def _serialize_pack_list(packs: List[GeneralPack]) -> list:
    """Convert GeneralPack objects to list of dicts for storage."""
    return [{
        "id": p.id,
        "base_template": p.base_template,
        "version": p.version,
...
def save_general_packs(packs: List[GeneralPack]) -> bool:
    from db import USE_DB
    if USE_DB:
        from db import save_general_packs_db
        save_general_packs_db(_serialize_pack_list(packs))
        return True
