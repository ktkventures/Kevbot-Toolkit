...
def prepare_data_with_indicators(
    ...
    if secondary_tfs and len(df) > 0:
        ...
        # store-swap (CONSUMER #2), the Phase-2b sub-minute canonical build, the 1-period
        # cross-TF shift, and the `_spec_` heatmap copy all live in the helper. Guarded by
        # the fidelity parity suite.
        ...
        try:
            if secondary_tf_dfs is not None and sec_tf in secondary_tf_dfs:
                ...
                if sec_df is None:
                    ...
                    # secondary is built from the canonical 1Sec-derived bars
                    # (store-served), NOT by upsampling the primary into a
                    # pseudo-series. None when the flag is off / tf >= 60 →
                    # byte-identical legacy resample below. Living in THIS
                    # helper means the M-RS5a resident frame gets the same
                    # canonical construction as the batch prep.
                    sec_df = _submin_secondary_canonical(
                        strat, sec_tf, primary_df, session)
                if sec_df is None:
