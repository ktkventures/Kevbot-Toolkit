...
const mockUserPacks: UserPack[] = [
  {
    ...
    parityScore: 100,
    params: [
      { key: 'period', label: 'RSI Period', value: 14, min: 2, max: 50, type: 'int' },
      { key: 'overbought', label: 'Overbought Level', value: 70, min: 50, max: 95, type: 'int' },
      { key: 'oversold', label: 'Oversold Level', value: 30, min: 5, max: 50, type: 'int' },
    ],
    plotSettings: [
      { key: 'rsi_color', label: 'RSI Line', default: '#8b5cf6' },
      { key: 'ob_color', label: 'Overbought Zone', default: '#ef4444' },
      { key: 'os_color', label: 'Oversold Zone', default: '#22c55e' },
    ],
    outputs: [
      { code: 'OB', description: 'Overbought -- RSI > 70' },
      { code: 'NEUTRAL', description: 'Neutral -- RSI between 30-70' },
      { code: 'OS', description: 'Oversold -- RSI < 30' },
    ],
    triggers: [
      { id: 'rsi_enter_ob', name: 'Enter Overbought', direction: 'SHORT', type: 'ENTRY', exec: '[C]' },
      { id: 'rsi_exit_ob', name: 'Exit Overbought', direction: 'LONG', type: 'ENTRY', exec: '[C]' },
      { id: 'rsi_enter_os', name: 'Enter Oversold', direction: 'LONG', type: 'ENTRY', exec: '[C]' },
      { id: 'rsi_exit_os', name: 'Exit Oversold', direction: 'SHORT', type: 'ENTRY', exec: '[C]' },
    ],
  },
  {
    id: 'bb-squeeze',
    ...
    parityScore: 98,
    params: [
      { key: 'bb_period', label: 'BB Period', value: 20, min: 5, max: 50, type: 'int' },
      { key: 'bb_std', label: 'BB StdDev', value: 2.0, min: 0.5, max: 4.0, type: 'float' },
      { key: 'kc_period', label: 'KC Period', value: 20, min: 5, max: 50, type: 'int' },
      { key: 'kc_mult', label: 'KC Multiplier', value: 1.5, min: 0.5, max: 4.0, type: 'float' },
    ],
    plotSettings: [
      { key: 'squeeze_on_color', label: 'Squeeze On', default: '#ef4444' },
      { key: 'squeeze_off_color', label: 'Squeeze Off', default: '#22c55e' },
    ],
    outputs: [
      { code: 'SQUEEZE_ON', description: 'BB inside KC -- low volatility squeeze' },
      { code: 'SQUEEZE_OFF_BULL', description: 'BB expanded outside KC, momentum positive' },
      { code: 'SQUEEZE_OFF_BEAR', description: 'BB expanded outside KC, momentum negative' },
    ],
    triggers: [
      { id: 'bb_squeeze_fire_bull', name: 'Squeeze Fire Bullish', direction: 'LONG', type: 'ENTRY', exec: '[C]' },
      { id: 'bb_squeeze_fire_bear', name: 'Squeeze Fire Bearish', direction: 'SHORT', type: 'ENTRY', exec: '[C]' },
    ],
  },
];
...
const mockStateTimelines: Record<string, StateTimelineEntry[]> = {
  'rsi-custom': [
    { time: '09:30:00', state: 'NEUTRAL', prevState: 'NEUTRAL' },
    { time: '09:45:00', state: 'OB', prevState: 'NEUTRAL' },
    { time: '10:15:00', state: 'NEUTRAL', prevState: 'OB' },
    { time: '10:45:00', state: 'OS', prevState: 'NEUTRAL' },
    { time: '11:15:00', state: 'NEUTRAL', prevState: 'OS' },
    { time: '12:00:00', state: 'OB', prevState: 'NEUTRAL' },
    { time: '13:30:00', state: 'NEUTRAL', prevState: 'OB' },
  ],
  'bb-squeeze': [
    { time: '09:30:00', state: 'SQUEEZE_ON', prevState: 'SQUEEZE_ON' },
    { time: '10:00:00', state: 'SQUEEZE_OFF_BULL', prevState: 'SQUEEZE_ON' },
    { time: '10:45:00', state: 'SQUEEZE_ON', prevState: 'SQUEEZE_OFF_BULL' },
    { time: '11:30:00', state: 'SQUEEZE_OFF_BEAR', prevState: 'SQUEEZE_ON' },
    { time: '12:15:00', state: 'SQUEEZE_ON', prevState: 'SQUEEZE_OFF_BEAR' },
  ],
};

const mockTriggerEvents: Record<string, TriggerEvent[]> = {
  'rsi-custom': [
    { time: '09:46:00', trigger: 'Enter Overbought', direction: 'SHORT', type: 'ENTRY', price: '$489.20', execType: '[C]' },
    { time: '10:16:00', trigger: 'Exit Overbought', direction: 'LONG', type: 'ENTRY', price: '$487.50', execType: '[C]' },
    { time: '10:46:00', trigger: 'Enter Oversold', direction: 'LONG', type: 'ENTRY', price: '$484.30', execType: '[C]' },
    { time: '11:16:00', trigger: 'Exit Oversold', direction: 'SHORT', type: 'ENTRY', price: '$486.10', execType: '[C]' },
  ],
  'bb-squeeze': [
    { time: '10:01:00', trigger: 'Squeeze Fire Bullish', direction: 'LONG', type: 'ENTRY', price: '$488.50', execType: '[C]' },
    { time: '11:31:00', trigger: 'Squeeze Fire Bearish', direction: 'SHORT', type: 'ENTRY', price: '$485.20', execType: '[C]' },
  ],
};

/* Built-in template packs for "Create from Template" */
const builtInTemplates = [
  { key: 'rsi', name: 'RSI', category: 'Momentum', description: 'Relative Strength Index with overbought/oversold zones' },
  { key: 'bollinger', name: 'Bollinger Bands', category: 'Volatility', description: 'Price bands with squeeze detection' },
  { key: 'stochastic', name: 'Stochastic', category: 'Momentum', description: 'Stochastic oscillator with K/D crossovers' },
  { key: 'atr', name: 'ATR', category: 'Volatility', description: 'Average True Range for volatility measurement' },
  { key: 'obv', name: 'OBV', category: 'Volume', description: 'On-Balance Volume trend analysis' },
];
              ...
              <Card>
                ...
                <div className="grid grid-cols-4 gap-4 mb-4">
                  {[
                    ...
                    { label: 'All States Reached', value: `${pack.outputs.length}/${pack.outputs.length}`, color: 'var(--green)' },
                  ].map((m) => (
                    <div key={m.label}>
                    ...
                    <tbody>
                      {pack.triggers.map((t) => (
                        <tr key={t.id} style={{ borderBottom: '1px solid var(--border)' }}>
                          ...
                          <td className="py-2 px-3"><span style={{ color: 'var(--green)' }}>✓</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <Card>
                ...
                <div className="grid grid-cols-4 gap-4 mt-4 mb-4">
                  {[
                    ...
                    { label: 'Matched', value: pack.parityScore === 100 ? '47' : '46', color: 'var(--green)' },
                    { label: 'Mismatched', value: pack.parityScore === 100 ? '0' : '1', color: pack.parityScore === 100 ? 'var(--green)' : 'var(--red)' },
                    { label: 'Parity Score', value: `${pack.parityScore || 100}%`, color: pack.parityScore === 100 ? 'var(--green)' : 'var(--orange)' },
                  ].map((m) => (
                    <div key={m.label}>
                      <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{m.label}</p>
                      <p className="text-lg font-bold" style={m.color ? { color: m.color } : undefined}>{m.value}</p>
...
function CreateTemplateModal({
  ...
  const tpl = builtInTemplates.find((t) => t.key === selectedTemplate) || builtInTemplates[0];
...
export default function UserPacksV5() {
  const [packs, setPacks] = useState<UserPack[]>(mockUserPacks);
        ...
        <Card>
          <div className="text-center py-16">
            ...
            <p className="text-sm mb-8 max-w-md mx-auto" style={{ color: 'var(--text-muted)' }}>
              Create your own indicator packs with custom parameters, outputs, and triggers.
              Start from scratch or use a built-in template as a starting point.
            </p>
