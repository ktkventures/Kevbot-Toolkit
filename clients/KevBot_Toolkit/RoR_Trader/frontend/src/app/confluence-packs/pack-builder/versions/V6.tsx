...
export default function PackBuilderV6() {
  ...
  // Step 2: Generated structure (populated after "Generate Structure")
  const [structureGenerated, setStructureGenerated] = useState(false);
  const [isGenerating, setIsGenerating] = useState(false);

  // Step 3: Refined structure (editable)
          ...
          {!structureGenerated ? (
            <Card>
              <div className="text-center py-12">
                {isGenerating ? (
                  <>
                    <div className="w-8 h-8 rounded-full border-2 border-t-transparent mx-auto mb-3 animate-spin" style={{ borderColor: 'var(--accent)', borderTopColor: 'transparent' }} />
                    <p className="text-sm" style={{ color: 'var(--accent)' }}>Generating structure from your description...</p>
                    <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>Proposing parameters, outputs, and triggers</p>
                  </>
                ) : (
                  <>
                    <p className="text-sm mb-2" style={{ color: 'var(--text-secondary)' }}>
                      Ready to generate the pack structure from your description.
                    </p>
                    <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>
                      The AI will propose parameters, output states, and triggers based on what you described. You can refine everything in the next step.
                    </p>
                    <button style={btnPrimary} onClick={handleGenerateStructure}>
                      Generate Structure
                    </button>
                  </>
                )}
            ...
            <Card>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="rounded-lg p-3" style={{ background: 'var(--bg-input)', border: '1px solid var(--border)' }}>
                  ...
                  {params.map((p) => (
                    <p key={p.id} className="text-[10px] font-mono" style={{ color: 'var(--text-muted)' }}>{p.name}: {p.type} = {p.defaultVal}</p>
                  ))}
                </div>
                <div className="rounded-lg p-3" style={{ background: 'var(--bg-input)', border: '1px solid var(--border)' }}>
                  <p className="text-xs font-medium mb-2">{outputs.length} Outputs</p>
                  {outputs.map((o) => (
                    <p key={o.id} className="text-[10px] font-mono" style={{ color: 'var(--text-muted)' }}>{o.code}</p>
                  ))}
                </div>
                <div className="rounded-lg p-3" style={{ background: 'var(--bg-input)', border: '1px solid var(--border)' }}>
              ...
              <p className="text-xs mt-3" style={{ color: 'var(--text-muted)' }}>
                Click Next to review and refine these details, or Regenerate if this isn&apos;t what you had in mind.
              </p>
            </Card>
          )}
        </div>
          ...
          <div className="col-span-7">
            ...
            <Card className="mb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button style={{ ...btnSecondary, fontSize: '0.75rem' }}>Copy Prompt to Clipboard</button>
              </div>
              <p className="text-xs mb-3" style={{ color: 'var(--text-muted)' }}>
                Copy the prompt above and paste it into Claude or ChatGPT. Then paste the full response below.
              </p>
              ...
              <button className="mt-2" style={{ ...btnPrimary, opacity: llmResponse.trim() ? 1 : 0.4 }}
                disabled={!llmResponse.trim()} onClick={handleParseCode}>
                Parse & Validate
              </button>
            </Card>
            ...
            {codeParsed && (
              <Card>
                <h4 className="text-sm font-medium mb-3">Parsed Code</h4>
          ...
          <Modal title="Request Fix" isOpen={showFixModal} onClose={() => setShowFixModal(false)} width="600px">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button style={{ ...btnPrimary, background: 'var(--orange)' }}
                onClick={() => { setFixCount(fixCount + 1); setShowFixModal(false); setFixDescription(''); }}>
                Generate Fix Prompt
              </button>
            </div>
          </Modal>
