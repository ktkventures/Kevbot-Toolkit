...
const MOCK_STOP_PACKS: RiskPack[] = [
...
const MOCK_TARGET_PACKS: RiskPack[] = [
...
function OptimizableVariables({
  ...
  const stopPack = MOCK_STOP_PACKS.find((p) => p.id === selectedStopPack);
  const targetPack = MOCK_TARGET_PACKS.find((p) => p.id === selectedTargetPack);
          ...
          <div className="flex items-center gap-3">
            ...
            {!configExpanded && entryDef && (
              <span className="text-xs font-normal" style={{ color: 'var(--text-muted)' }}>
                ...
                | Stop: {MOCK_STOP_PACKS.find((p) => p.id === selectedStopPack)?.summary || 'None'}{' '}
                | Target: {MOCK_TARGET_PACKS.find((p) => p.id === selectedTargetPack)?.summary || 'None'}
              </span>
            )}
          </div>
          <span
                    ...
                    if (tab === 'Stop Loss') {
                      ...
                      return (
                        <div>
                          ...
                          <p className="text-xs mb-3" style={{ color: 'var(--text-muted)' }}>
                            Current: <strong>{MOCK_STOP_PACKS.find((p) => p.id === selectedStopPack)?.summary || 'None'}</strong>
                          </p>
                          <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 440 }}>
                    ...
                    if (tab === 'Take Profit') {
                      ...
                      return (
                        <div>
                          ...
                          <p className="text-xs mb-3" style={{ color: 'var(--text-muted)' }}>
                            Current: <strong>{MOCK_TARGET_PACKS.find((p) => p.id === selectedTargetPack)?.summary || 'None'}</strong>
                          </p>
                          <div className="space-y-2 overflow-y-auto" style={{ maxHeight: 440 }}>
