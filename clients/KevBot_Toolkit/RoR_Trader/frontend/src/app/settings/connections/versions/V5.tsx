...
export default function SettingsConnectionsV5() {
      ...
      <Card className="mt-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs px-2 py-0.5 rounded-full" style={{ color: 'var(--green)', background: 'var(--green)' + '20' }}>
              Running
            </span>
          </div>
          <div className="flex gap-2">
            ...
            <button className="px-3 py-1.5 rounded-lg text-xs font-medium" style={{ background: 'var(--red-muted)', color: 'var(--red)' }}>
              Disable Monitoring
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-5 gap-4 mb-4">
          <div>
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Uptime</p>
            <p className="text-sm font-bold">4h 32m</p>
          </div>
          <div>
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>Ticks</p>
            <p className="text-sm font-bold font-mono">12,847</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-sm font-bold font-mono">10:32:15 AM</p>
          </div>
        </div>
        ...
        <div className="rounded-lg overflow-hidden" style={{ background: '#0d1117', maxHeight: 200, overflowY: 'auto' }}>
          <div className="p-3 font-mono text-xs space-y-1">
            {[
              { time: '10:32:15', level: 'INFO', msg: '[SPY] Entry signal fired — [C] EMA Bull Cross, all confluence met' },
              { time: '10:15:03', level: 'INFO', msg: '[NVDA] Entry signal fired — [C] EMA Bull Cross, RVOL HIGH' },
              { time: '09:58:44', level: 'INFO', msg: '[AAPL] Exit signal — +1.2R, target reached' },
              { time: '09:45:22', level: 'WARN', msg: '[META] Webhook delivery failed — timeout after 5s, retry queued' },
              { time: '09:38:11', level: 'INFO', msg: '[SPY] Exit signal — +0.85R, bar count exit' },
              { time: '09:31:00', level: 'INFO', msg: 'RTH session started — monitoring 4 strategies on 4 symbols' },
              { time: '09:30:05', level: 'INFO', msg: 'WebSocket connected to Polygon.io — SIP feed' },
              { time: '09:30:00', level: 'INFO', msg: 'Ralph Engine started — worker PID 48291' },
            ].map((log, i) => (
              <div key={i} className="flex gap-2">
                <span style={{ color: '#484f58' }}>{log.time}</span>
                <span style={{ color: log.level === 'WARN' ? '#d29922' : log.level === 'ERROR' ? '#f85149' : '#58a6ff', fontWeight: 600 }}>[{log.level}]</span>
