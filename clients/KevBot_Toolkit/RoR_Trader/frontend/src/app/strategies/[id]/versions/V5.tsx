...
const strategy = {
  ...
  updatedAt: '2026-03-18 10:15:00',
};

const mockTrades = [
  { id: 1, entryTime: '2026-03-18 09:42:15', exitTime: '2026-03-18 09:46:32', entryPrice: 142.35, exitPrice: 142.98, pnlR: 1.42, execType: '[C]', exitReason: 'Signal', isFwd: true },
  { id: 2, entryTime: '2026-03-18 10:15:03', exitTime: '2026-03-18 10:19:18', entryPrice: 143.10, exitPrice: 142.65, pnlR: -0.85, execType: '[C]', exitReason: 'Stop', isFwd: true },
  { id: 3, entryTime: '2026-03-18 11:02:44', exitTime: '2026-03-18 11:06:44', entryPrice: 143.50, exitPrice: 144.12, pnlR: 2.07, execType: '[L]', exitReason: 'Target', isFwd: true },
  { id: 4, entryTime: '2026-03-17 09:35:22', exitTime: '2026-03-17 09:39:22', entryPrice: 141.80, exitPrice: 141.45, pnlR: -0.92, execType: '[C]', exitReason: 'Stop', isFwd: true },
  { id: 5, entryTime: '2026-03-17 10:22:11', exitTime: '2026-03-17 10:26:11', entryPrice: 141.60, exitPrice: 142.25, pnlR: 1.63, execType: '[LC]', exitReason: 'Signal', isFwd: true },
  { id: 6, entryTime: '2026-03-17 11:45:08', exitTime: '2026-03-17 11:49:08', entryPrice: 142.90, exitPrice: 143.55, pnlR: 1.95, execType: '[C]', exitReason: 'Target', isFwd: true },
  { id: 7, entryTime: '2026-03-14 09:48:55', exitTime: '2026-03-14 09:52:55', entryPrice: 140.50, exitPrice: 141.30, pnlR: 2.38, execType: '[C]', exitReason: 'Signal', isFwd: false },
  { id: 8, entryTime: '2026-03-14 10:30:12', exitTime: '2026-03-14 10:34:12', entryPrice: 141.10, exitPrice: 140.78, pnlR: -0.53, execType: '[CC]', exitReason: 'Stop', isFwd: false },
  { id: 9, entryTime: '2026-03-13 09:32:18', exitTime: '2026-03-13 09:36:18', entryPrice: 139.80, exitPrice: 140.45, pnlR: 1.15, execType: '[C]', exitReason: 'Signal', isFwd: false },
  { id: 10, entryTime: '2026-03-13 10:55:22', exitTime: '2026-03-13 10:59:22', entryPrice: 140.30, exitPrice: 139.90, pnlR: -0.78, execType: '[L]', exitReason: 'Stop', isFwd: false },
  { id: 11, entryTime: '2026-03-12 09:40:05', exitTime: '2026-03-12 09:44:05', entryPrice: 138.90, exitPrice: 139.65, pnlR: 1.88, execType: '[C]', exitReason: 'Target', isFwd: false },
  { id: 12, entryTime: '2026-03-12 11:10:33', exitTime: '2026-03-12 11:14:33', entryPrice: 139.20, exitPrice: 138.80, pnlR: -0.67, execType: '[LC]', exitReason: 'Bar Count', isFwd: false },
];

const extendedKPIs = {
  // Performance
  wins: 121, losses: 103, bestTrade: 5.23, worstTrade: -2.10,
  avgWin: 1.85, avgLoss: -0.90, payoffRatio: 2.06, expectedDailyR: 1.95,
  // Risk-Adjusted
  sharpe: 1.82, sortino: 2.65, calmar: 4.12, kelly: 0.182,
  dailyVaR: -1.42, cvar: -1.98, volatility: 12.4, rSquared: 0.91,
  // Distribution
  skewness: 0.34, kurtosis: 2.18, tailRatio: 1.45, outlierWinPct: 2.8, outlierLossPct: 2.3,
  // Drawdown
  maxRDD: -2.5, recoveryFactor: 20.9, ulcerIndex: 0.032, serenityIndex: 2.14,
...
function SigmaBadge({ label, value, color }: { label: string; value: number; color: string }) {
  ...
  return (
    ...
    >
      {label} {value >= 0 ? '+' : ''}{value.toFixed(1)}&sigma;
    </span>
  );
}
...
export default function StrategyDetailV5() {
  ...
  const alertAccuracy = strategy.alertTrades > 0
    ? ((strategy.alertWinRate / strategy.fwdWinRate) * 100).toFixed(1)
    : '--';

  const fwdTrades = mockTrades.filter((t) => t.isFwd);
  const btTrades = mockTrades.filter((t) => !t.isFwd);
  ...
  function renderKpiComparison(
    ...
    dataA: { wr: number; pf: number; dr: number; droi: number; tpd: number; mdd: number },
    dataB: { wr: number; pf: number; dr: number; droi: number; tpd: number; mdd: number },
  ) {
    const rows = [
      { label: 'Win Rate', a: `${dataA.wr.toFixed(1)}%`, b: `${dataB.wr.toFixed(1)}%`, d: dataB.wr - dataA.wr, fmt: (v: number) => `${v >= 0 ? '+' : ''}${v.toFixed(1)}%` },
      { label: 'PF', a: dataA.pf.toFixed(2), b: dataB.pf.toFixed(2), d: dataB.pf - dataA.pf, fmt: (v: number) => `${v >= 0 ? '+' : ''}${v.toFixed(2)}` },
      { label: 'Daily R', a: `${dataA.dr >= 0 ? '+' : ''}${dataA.dr.toFixed(2)}`, b: `${dataB.dr >= 0 ? '+' : ''}${dataB.dr.toFixed(2)}`, d: dataB.dr - dataA.dr, fmt: (v: number) => `${v >= 0 ? '+' : ''}${v.toFixed(2)}` },
      { label: 'Daily ROI', a: `${dataA.droi.toFixed(2)}%`, b: `${dataB.droi.toFixed(2)}%`, d: dataB.droi - dataA.droi, fmt: (v: number) => `${v >= 0 ? '+' : ''}${v.toFixed(2)}%` },
      { label: 'TPD', a: dataA.tpd.toFixed(1), b: dataB.tpd.toFixed(1), d: dataB.tpd - dataA.tpd, fmt: (v: number) => `${v >= 0 ? '+' : ''}${v.toFixed(1)}` },
      { label: 'Max DD', a: `${dataA.mdd.toFixed(1)}R`, b: `${dataB.mdd.toFixed(1)}R`, d: dataB.mdd - dataA.mdd, fmt: (v: number) => `${v >= 0 ? '+' : ''}${v.toFixed(1)}R`, invert: true },
    ];
  ...
  function renderTradeTable(trades: typeof mockTrades) {
    return (
      <div style={{ overflowX: 'auto' }}>
        <table className="w-full text-sm" style={{ borderCollapse: 'collapse' }}>
          <thead>
