...
const ENTRY_TRIGGERS = [
  { id: 'ema-bull-cross', name: 'EMA Bull Cross', tag: '[C]', pack: 'EMA Stack' },
  { id: 'ema-bear-cross', name: 'EMA Bear Cross', tag: '[C]', pack: 'EMA Stack' },
  { id: 'ema-fan-bull', name: 'EMA Fan Open Bull', tag: '[C]', pack: 'EMA Stack' },
  { id: 'macd-cross-bull', name: 'MACD Cross Bull', tag: '[C]', pack: 'MACD Line' },
  { id: 'macd-cross-bear', name: 'MACD Cross Bear', tag: '[C]', pack: 'MACD Line' },
  { id: 'vwap-cross-up', name: 'VWAP Cross Above', tag: '[L0]', pack: 'VWAP' },
  { id: 'vwap-cross-down', name: 'VWAP Cross Below', tag: '[L0]', pack: 'VWAP' },
  { id: 'ut-bot-buy', name: 'UT Bot Buy', tag: '[HM]', pack: 'UT Bot' },
  { id: 'ut-bot-sell', name: 'UT Bot Sell', tag: '[HM]', pack: 'UT Bot' },
];

const EXIT_TRIGGERS = [
  { id: 'ema-bear-cross-exit', name: 'EMA Bear Cross', tag: '[C]', pack: 'EMA Stack' },
  { id: 'ema-bull-cross-exit', name: 'EMA Bull Cross', tag: '[C]', pack: 'EMA Stack' },
  { id: 'macd-cross-bear-exit', name: 'MACD Cross Bear', tag: '[C]', pack: 'MACD Line' },
  { id: 'vwap-cross-below-exit', name: 'VWAP Cross Below', tag: '[L0]', pack: 'VWAP' },
];

const TF_CONFLUENCES = [
  { id: '_TF_-EMA_STACK-SML', display: 'EMA_STACK: SML', pack: 'EMA Stack' },
  { id: '_TF_-EMA_STACK-LMS', display: 'EMA_STACK: LMS', pack: 'EMA Stack' },
  { id: '_TF_-MACD_LINE-M>S+', display: 'MACD_LINE: M>S+', pack: 'MACD Line' },
  { id: '_TF_-MACD_LINE-M<S-', display: 'MACD_LINE: M<S-', pack: 'MACD Line' },
  { id: '_TF_-VWAP_POSITION->V', display: 'VWAP_POSITION: >V', pack: 'VWAP' },
  { id: '_TF_-VWAP_POSITION-<V', display: 'VWAP_POSITION: <V', pack: 'VWAP' },
  { id: '_TF_-RVOL-HIGH', display: 'RVOL: HIGH', pack: 'RVOL' },
];

const GENERAL_CONFLUENCES = [
  { id: 'GEN-time-1-RTH_OPEN', display: 'Time of Day: RTH Open', pack: 'Time of Day' },
  { id: 'GEN-time-1-MIDDAY', display: 'Time of Day: Midday', pack: 'Time of Day' },
  { id: 'GEN-day-1-MON_TUE_WED', display: 'Day of Week: Mon-Wed', pack: 'Day of Week' },
];
...
export default function MassBuilderV2() {
  // Config state
      ...
      <Card className="mb-5">
        ...
        <div className="min-h-[180px]">
          ...
          {activeConfigTab === 'Entry' && (
            <div>
              <p className="text-sm font-medium mb-3" style={{ color: 'var(--text-secondary)' }}>
                Select entry triggers to test
              </p>
              {/* Group by pack */}
              {Object.entries(
                ENTRY_TRIGGERS.reduce<Record<string, typeof ENTRY_TRIGGERS>>((acc, t) => {
                  (acc[t.pack] = acc[t.pack] || []).push(t);
                  return acc;
                }, {})
              ).map(([pack, triggers]) => (
                <div key={pack} className="mb-3">
                  <p className="text-xs font-medium mb-1.5" style={{ color: 'var(--text-muted)' }}>{pack}</p>
                  <div className="space-y-1">
                    {triggers.map((t) => (
                      <label key={t.id} className="flex items-center gap-2 cursor-pointer py-0.5">
                        <input
                          type="checkbox"
                          checked={selectedEntries.includes(t.id)}
            ...
            <div>
              ...
              <div className="space-y-1 mb-4">
                {EXIT_TRIGGERS.map((t) => (
                  <label key={t.id} className="flex items-center gap-2 cursor-pointer py-0.5">
                    <input
                      type="checkbox"
                      checked={selectedExits.includes(t.id)}
                      onChange={() => toggleItem(selectedExits, t.id, setSelectedExits)}
                      className="rounded"
                    />
                    <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                      {t.name}
              ...
              <div className="border-t pt-3" style={{ borderColor: 'var(--border)' }}>
                ...
                <p className="text-[10px] mt-1" style={{ color: 'var(--text-muted)' }}>
                  Depth 1 = test individually. 2+ = test combinations.
                </p>
              </div>
            </div>
          )}
          ...
          {activeConfigTab === 'TF Confluence' && (
            <div>
              <p className="text-sm font-medium mb-3" style={{ color: 'var(--text-secondary)' }}>
                Select TF confluences for auto-search
              </p>
              {Object.entries(
                TF_CONFLUENCES.reduce<Record<string, typeof TF_CONFLUENCES>>((acc, c) => {
                  (acc[c.pack] = acc[c.pack] || []).push(c);
                  return acc;
                }, {})
              ).map(([pack, confs]) => (
                <div key={pack} className="mb-3">
                  ...
                  <div className="space-y-1">
                    {confs.map((c) => (
                      <label key={c.id} className="flex items-center gap-2 cursor-pointer py-0.5">
                        <input
                          type="checkbox"
                          checked={selectedTfConf.includes(c.id)}
                          onChange={() => toggleItem(selectedTfConf, c.id, setSelectedTfConf)}
                          className="rounded"
                        />
                  ...
                  <div className="space-y-1">
                    {confs.map((c) => (
                      <label key={c.id} className="flex items-center gap-2 cursor-pointer py-0.5">
                        <input
                          type="checkbox"
                          checked={selectedGenConf.includes(c.id)}
                          onChange={() => toggleItem(selectedGenConf, c.id, setSelectedGenConf)}
                          className="rounded"
                        />
                        <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                          {c.display}
          ...
          {activeConfigTab === 'Stop Loss' && (
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {STOP_METHODS.map((m) => (
                  <ToggleChip key={m} label={m} active={stopMethod === m} onClick={() => setStopMethod(m)} />
                ))}
              </div>
              {stopMethod === 'ATR' && (
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: 'var(--text-muted)' }}>ATR Multiplier</label>
                  <input
                    type="number" min={0.5} max={5} step={0.1}
                    value={stopAtrMult}
                    onChange={(e) => setStopAtrMult(parseFloat(e.target.value) || 1.5)}
                    className="w-24 px-3 py-1.5 rounded-lg text-sm"
                    style={{ background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}
                  />
                </div>
              ...
              {stopMethod === 'Fixed $' && (
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: 'var(--text-muted)' }}>Dollar Amount</label>
                  <input
              ...
              {stopMethod === 'Pct %' && (
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: 'var(--text-muted)' }}>Percentage</label>
                  <input
              ...
              {stopMethod === 'Swing' && (
                <div className="flex gap-4">
                  <div>
                    <label className="text-xs font-medium block mb-1" style={{ color: 'var(--text-muted)' }}>Lookback</label>
                    <input
                  ...
                  <div>
                    <label className="text-xs font-medium block mb-1" style={{ color: 'var(--text-muted)' }}>Padding $</label>
                    <input
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {TARGET_METHODS.map((m) => (
                  <ToggleChip key={m} label={m} active={targetMethod === m} onClick={() => setTargetMethod(m)} />
                ))}
              </div>
              {targetMethod === 'R:R' && (
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: 'var(--text-muted)' }}>Risk:Reward Ratio</label>
                  <input
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
        ...
        <Card className="md:col-span-2">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs" style={{ color: 'var(--text-secondary)' }}>
            ...
            <span>{estimate.nDirs} dir{estimate.nDirs !== 1 ? 's' : ''}</span>
            <span>{estimate.nEntries} entr{estimate.nEntries !== 1 ? 'ies' : 'y'}</span>
            <span>{estimate.nExits} exit{estimate.nExits !== 1 ? 's' : ''}</span>
          </div>
          <div className="mt-2 flex items-baseline gap-3">
            ...
            <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
              Est. {formatTime(estimate.estSeconds)}
            </span>
          </div>
          <p className="text-xs mt-1" style={{ color: 'var(--text-muted)' }}>
            Data range: Last {lookbackDays} days
          </p>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Min Trades</span>
              <input
                type="number" min={1} max={500}
                value={minTrades}
                onChange={(e) => setMinTrades(parseInt(e.target.value, 10) || 10)}
                className="w-16 px-2 py-1 rounded text-xs text-right"
                style={{ background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}
              />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Min WR %</span>
              <input
                type="number" min={0} max={100} step={5}
                value={minWR}
                onChange={(e) => setMinWR(parseFloat(e.target.value) || 0)}
                className="w-16 px-2 py-1 rounded text-xs text-right"
                style={{ background: 'var(--bg-input)', border: '1px solid var(--border)', color: 'var(--text-primary)' }}
              />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Min PF</span>
              <input
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs" style={{ color: 'var(--text-muted)' }}>Max Results</span>
              <input
