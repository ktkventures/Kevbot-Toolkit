...
/**
 * ParityDriftRibbon — per-bar parity "microscope" for the Gate Parity tab.
 *
 * Joins the two raw bar pipelines by timestamp and renders one horizontal
 * ribbon per metric so drift between the Backtest model (REST / rest_hifi) and
 * the Live model (ws cache) is visible at a glance — BEFORE it ever turns into
 * a different entry/exit.
 *
 *   Backtest bars = chartDataResp.chart_data   (REST)
 *   Live bars     = labCacheLatest.chart_data  (ws cache)
 *   Both share the same column schema (built by the same buildStrategyChartPanes
 *   metadata), so we compare field-for-field at matched bar timestamps.
 *
 * Rows: Bar exists · Open · High · Low · Close · each indicator · each gate state.
 * Cell colors: green = match · yellow = minor drift · red = major drift ·
 *              gray = missing on one side / not comparable.
 *
 * Frontend-only, scrub-independent (covers the whole loaded window). Hover a
 * cell for that bar's time + both values + Δ.
 */

import { useMemo, useState } from 'react';

// ---- tolerances (also surfaced on-screen) ----
const PRICE_ABS_MATCH = 0.01;    // ≤ 1 cent always counts as a match
const PRICE_REL_MATCH = 0.0002;  // 0.02%
const PRICE_REL_MINOR = 0.001;   // 0.1%
const NUM_ABS_MATCH = 1e-6;
const NUM_REL_MATCH = 0.001;     // 0.1%
const NUM_REL_MINOR = 0.01;      // 1%
const MAX_BARS = 300;            // cap ribbon width; note truncation if exceeded
...
interface RowResult {
  ...
  minor: number;
  major: number;
  comparable: number; // match+minor+major (excludes na)
}
...
export default function ParityDriftRibbon({
  ...
  const result = useMemo(() => {
    ...
    if (bt.length === 0 || al.length === 0) {
      return null;
    }

    // Sort each side by time, index by unix-sec, keep prev pointers for PB states.
    const sortByTime = (arr: any[]) =>
      [...arr].sort((x, y) => toUnixSec(x.timestamp) - toUnixSec(y.timestamp));
    const btS = sortByTime(bt);
    const alS = sortByTime(al);
    const btIdx = new Map<number, number>();
    btS.forEach((b, i) => btIdx.set(toUnixSec(b.timestamp), i));
    const alIdx = new Map<number, number>();
    alS.forEach((b, i) => alIdx.set(toUnixSec(b.timestamp), i));

    // Clip to the OVERLAPPING time range so the backtest's deep history isn't
    // counted as false "backtest-only" bars — drift only means anything where
    // BOTH pipelines cover the same window. (The cache is usually the limiter.)
    const minMax = (m: Map<number, number>) => {
      let lo = Infinity, hi = -Infinity;
      m.forEach((_, t) => { if (t < lo) lo = t; if (t > hi) hi = t; });
    ...
    // Union of bar times within the overlap (sorted), capped to recent MAX_BARS.
    ...
    for (const cond of heatmapConds) {
      ...
      defs.push({
        key: `state:${cond.column}`,
        label: `${pretty(cond.label || cond.column)} ⟨state⟩`,
        kind: 'state',
        col: cond.column,
    ...
    // State resolver honoring PB (previous closed bar) fidelity, per side.
    ...
    const rows: RowResult[] = defs.map((def) => {
      ...
      for (const t of times) {
        ...
        if (def.kind === 'exists') {
          ...
          tip = `${when}\nbacktest: ${inBt ? 'bar' : '—'} · live: ${inAl ? 'bar' : '—'}`;
        } else if (!bBar || !aBar) {
          status = 'na';
  ...
  if (!result) {
    return (
      <p className="text-xs py-2" style={{ color: 'var(--text-muted)' }}>
        Need both lenses loaded to compare — backtest bars: <strong>{backtestBars?.length || 0}</strong>,
        {' '}live cache bars: <strong>{alertBars?.length || 0}</strong>.
        {(alertBars?.length || 0) === 0 && ' (No live cache rows for this strategy/window yet.)'}
      </p>
    );
  }
    ...
    <div className="space-y-2">
      ...
      <div className="rounded" style={{ border: '1px solid var(--border)', overflow: 'hidden' }}>
        {rows.map((row, ri) => {
          ...
          const pctColor = pct == null ? 'var(--text-muted)'
            : pct >= 99 ? 'var(--green)' : pct >= 90 ? 'rgba(234,179,8,0.95)' : 'var(--red)';
          return (
            <div
              key={row.def.key}
            ...
            >
              ...
              <div
                ...
                style={{ width: 210, background: 'var(--bg-input)', fontSize: 11 }}
                title={row.def.kind === 'state' ? `gate/confluence state (${row.def.fidelity})` : row.def.kind}
              >
                ...
                {row.cells.map((c, ci) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRight: row.cells.length <= 120 ? '1px solid rgba(0,0,0,0.15)' : 'none',
                      cursor: 'crosshair',
                    }}
                  />
                ))}
      ...
      <div className="flex items-start justify-between gap-3 text-[10px]" style={{ color: 'var(--text-muted)' }}>
        <pre className="whitespace-pre-wrap m-0" style={{ fontFamily: 'inherit', minHeight: 14 }}>
          {hover || 'Hover a cell for that bar’s time + both values + Δ.'}
        </pre>
