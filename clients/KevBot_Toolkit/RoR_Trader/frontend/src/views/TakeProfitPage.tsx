...
/**
 * Take Profit Packs — Clean API-first page.
 *
 * Visual design adapted from V1 (take-profit/versions/V1.tsx), data layer built
 * around actual Supabase API response shapes. No mock data, no fallbacks.
 *
 * Filters risk management packs to show only target-related templates
 * (those with target_method on the template, or known target template keys).
 */
...
/** Known target-related template keys */
const TARGET_TEMPLATES = new Set([
  'risk_reward', 'rr_ratio', 'atr_target', 'fixed_dollar_target',
  'percentage_target', 'swing_target', 'no_target', 'none',
]);

function isTargetTemplate(templateKey: string, template?: RMTemplateDTO): boolean {
  // Primary check: template has target_method
  if (template?.target_method) return true;
  // Fallback: known target template names
  if (TARGET_TEMPLATES.has(templateKey)) return true;
  // Exclude templates that are stop-only
  if (template?.stop_method && !template?.target_method) return false;
  return false;
}
...
export default function TakeProfitPage() {
  ...
  // Filter to target-only packs
  const targetPacks = useMemo(() => {
    if (!allPacks || !templates) return [];
    return allPacks.filter((p) => isTargetTemplate(p.base_template, templates[p.base_template]));
  }, [allPacks, templates]);

  // Search filter
  const filtered = useMemo(() => {
    if (!search.trim()) return targetPacks;
    const q = search.toLowerCase();
    return targetPacks.filter((p) => {
  ...
  function handleToggle(pack: RiskManagementPackDTO) {
    ...
    // Save ALL risk management packs (not just target packs) so we don't lose stops
    if (allPacks) {
      ...
      <div>
        ...
        <Card>
          <div className="text-center py-8" style={{ color: 'var(--red)' }}>
            Failed to load take profit packs. Check your connection and try again.
          </div>
        </Card>
      </div>
    );
  ...
  const enabledCount = targetPacks.filter((p) => isEnabled(p)).length;
  const totalCount = targetPacks.length;

  return (
    <div>
      <PageHeader
        title="Take Profit Packs"
      ...
      <div className="flex flex-wrap gap-2 mb-4 mt-4">
        <input
          type="text"
          placeholder="Search take profit packs..."
          value={search}
        ...
        <Card>
          <div className="text-center py-12">
            <p className="text-lg mb-2" style={{ color: 'var(--text-secondary)' }}>
              {totalCount === 0
                ? 'No take profit packs configured yet.'
