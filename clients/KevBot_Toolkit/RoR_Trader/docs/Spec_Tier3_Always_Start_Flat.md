...
**Status:** DRAFT (2026-05-20). Implementation-level spec for the
"always start flat" unification of full and rapid backtest semantics.
...
This spec sits at the top of the "what's left" stack — needs Kevin
sign-off on the resolved decisions in §6 before any code lands.
...
## 6. Decisions to resolve before code lands

These need Kevin's explicit sign-off before any implementation begins.
...
trigger ID + same FLAT→entry resolution. Is that the intended
behavior, or do we want each webhook to fire regardless? (Lean:
dedupe.)

### 6.2 The carryover position log

§4.2 proposes recording a `position_carryover` entry per
strategy-in-open-position at rollout. Does Kevin want this surfaced
to the user (Data Fidelity tab amber banner) or just engine-side
audit? (Lean: user-surfaced; this affects real money.)
...
The proposed rule is cleaner; the alternative might be more
familiar to traders. (Lean: proposed rule.)

### 6.4 The "rapid backtest" path

After Tier 3, the rapid (windowed) and full (replay) backtests
differ only in window length + stop fidelity. Decision: collapse
them to a single path with a `fidelity` parameter, OR keep them
distinct for UI clarity? (Lean: collapse; the UI can still expose
a "Recompute from scratch" button that sets fidelity to maximum.)

### 6.5 The chart rendering boundary

When the chart pane shows a date range that includes a strategy's
...
"owned" by the pre-boundary window. Should the chart render it as a
trade marker anyway (using its known entry timestamp), or drop it?
(Lean: render — the chart is a historical record, not a strategy
performance view.)

## 7. Validation

Once §6 decisions are locked and code lands, three integration tests
gate the rollout:
...
ship in parallel once 8.1 lands.

### 8.1 Lock decisions (no code)

Resolve §6.1–6.5 with Kevin. Write resolutions inline in this spec
under each decision. Spec becomes the authoritative reference for
the implementation.
