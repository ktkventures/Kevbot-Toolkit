...
### Coarse (1H/4H/1D) confluence gates never fire live → gated strategies silent (HIGH)
- **Status:** FIX DEPLOYED 2026-06-26 (commit a38f876), VERIFYING live. Kill-switch `RORT_SEED_MTF_FROM_WARMUP=0`.
- **Symptom:** strategies with a ≥1Hour confluence gate fire **0 live alerts** while their backtest fires normally. Confirmed real (not "correctly quiet"): 310 (1d-SUPERTREND) = 77 backtest trades in the live window / 0 alerts; 313 (1d-UT_BOT_V4) = 85 / 0. (312, also 1d-gated, IS correctly quiet — its daily condition genuinely hasn't occurred: 0 backtest in live window too. So the discriminator is "1d gate AND the daily condition is met.")
- **Root cause:** `SymbolHub._mtf_confluence[tf]` (the cross-TF gate-state dict the entry trigger checks) is populated **only** on a live `shadow.on_bar_close` — i.e. when a bar of that TF *closes live*. Coarse secondary TFs (1Hour/4Hour/**1Day**) almost never close intraday (a 1Day bar closes once/day), so their state stays **absent** → the gate's `need=['1D-…']` is never in `present` → `subset_ok=0` forever → every entry blocked. Visible in `XTF_BLOCK_DIAG`: `mtf_keys` topped out at 900–1800s (15–30M); no 3600/14400/86400. The Bug-5 warmup (2026-06-18) correctly seeds the coarse **indicator** (246 daily bars, "0/12 mismatches" vs backtest) but never derives that into `_mtf_confluence`.
- **Fix:** new `SymbolHub.seed_mtf_from_shadow(tf)` — after `shadow.warmup`, derive the confluence from the warmed indicator state (`_derive_confluence_records`, no extra pass) and publish to `_mtf_confluence[tf]`, so coarse gates are evaluable immediately (matching backtest's last-closed coarse bar). Called in `_warmup_all` (startup) + worker hot-reload (`ralph_engine.py`/`worker.py`). NOTE follow-up: the worker hot-reload still loads `days=7` (starves a 1Day shadow ~5 bars); startup `_warmup_all` uses the Bug-5 TF-scaled load — widen the hot-reload path too.
- **Verify:** worker log `[MTF-SEED] … tf=86400`; `XTF_BLOCK_DIAG` `present` now includes `1D-…`; 310/313 start firing live in line with backtest.

### REST Bars cache UPSAMPLED 1Min→sub-minute → wrecked 10Sec/15Sec backtests (SEV)
- **Status:** ✅ FIXED-VERIFIED (2026-06-26, commit b8bac23). End-to-end confirmed with cache **re-enabled**: UAD on 275 (10Sec) + 321 (15Sec) reproduced their cache-off backtests **byte-identical** — all 215/842 prior trades present, 0 dropped, only natural right-edge additions (not the corrupted 29/198). Logs show the cache served **1Sec** (`[HIFI] load-once … from REST Bars`), no errors, 98s for both. Cache is on + safe for sub-minute.
