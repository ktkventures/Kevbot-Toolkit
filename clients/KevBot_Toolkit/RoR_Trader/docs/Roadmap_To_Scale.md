...
- [ ] 8.7-hifi-signal-exits. [Required] **Hi-Fi signal-exit refinement** — extend Hi-Fi Pass 2 to refine L-type SIGNAL exits (e.g. `eppv4_cross_mid_down_ib`), not just stop_loss/target. Today's Hi-Fi only handles stop/target via `_walk_1s_for_exit` because those have known fixed levels; signal exits require the user pack to declare what level they cross at sub-second resolution. **DESIGN IN PROGRESS 2026-05-07** — preserves user-pack-as-source-of-truth contract (pack declares its own crossing predicate).
