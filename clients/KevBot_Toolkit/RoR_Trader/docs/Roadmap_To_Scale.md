...
- [ ] 8.7-post-migration-tasks. [Open as of 2026-05-11 PM] Follow-ups now that Phase 41 is live:
  - [ ] Run Update All Data on each of the 19 strategies (~4 min/strategy, ~75 min total) to extend backtest lane through today
  - [ ] Decide whether to re-enable cron (`ALGO_HISTORY_CRON_ENABLED=true` on Railway worker) or stay manual
  - [ ] Tag the 23 NULL data_source rows that landed in trades table during the migration window (auto-resolves on next cron cycle once enabled, OR one-liner UPDATE)
  - [ ] Track per-strategy REST↔CACHE entry drift across all 19 strategies; flag any with drift > 10s on 1Min as fidelity issues worth investigating
  - [ ] Eventually deprecate `strategies.stored_trades` JSONB column (writes are skipped; column is now vestigial). Defer until 1+ week of stable operation.
  - [ ] Re-investigate Hi-Fi Phase 2 (X exec_type for indicator-vs-indicator crosses) — exit drift (~7min median on sid 152) is the remaining fidelity gap. With backtest lane now reliable, Phase 2 has a clearer baseline to measure against.
