...
- [ ] 8.7-stored-trades-staleness. [Investigate Monday] **stored_trades stuck at April dates after bulk Update All Data** observed 2026-05-09. All 19 strategies' `stored_trades` show latest entry from April 9-30 despite `last_recompute_until_ts` stamped today (20:11-20:15 UTC). Algo lane (trades table, cache_locked) refreshed correctly to today; backtest lane (stored_trades, rest_hifi) appears to have silently failed-soft on the whole bulk run. Hypothesis: rest_hifi engine timed out under Supabase intermittent issues during the outage day, OR the new `recompute_and_persist_algo_trades` helper is overwriting stored_trades when it shouldn't, OR `update_strategy_admin` is being filtered. Investigate Monday with healthy Supabase. Non-urgent — algo + live lanes still work; KPIs may be slightly stale.
- [ ] 8.7-rest-vs-cache-divergence-v2. [Future] **Real REST-vs-CACHE divergence** — design doc shipped 2026-05-08 at `docs/Design_REST_vs_CACHE_Divergence_v2.md`. Locks in Option B (JSONB map keyed by model). ~5h core + 1h cutover. Phased: schema migration → dual-write on existing recompute → under-model endpoint → 4th lane in Divergence tab → eventually deprecate top-level stored_trades. Not yet scheduled.
